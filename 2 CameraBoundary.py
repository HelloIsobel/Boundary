...
if __name__ == "__main__":
    ...
    T_contour_area = 10000  # TODO：二值分割后，轮廓面积最小阈值
    T_area = 15000  # TODO：第二条线，顶端田埂面积阈值
    TestNum = "data/result/test0301/"  # TODO：本次实验保存地址
    if not os.path.exists(TestNum):
        os.mkdir(TestNum)

    candidateBoundary_dir = TestNum + "candidateBoundary/"
    if not os.path.exists(candidateBoundary_dir):
        os.mkdir(candidateBoundary_dir)

    runningtime = []
    runningtime_excelpath = TestNum + 'camera_runningtime.csv'
